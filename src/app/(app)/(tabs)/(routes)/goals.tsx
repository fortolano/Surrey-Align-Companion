  ...
  return (
    <Animated.View entering={FadeInDown.duration(350).delay(80 + index * 60)}>
      ...
      >
        ...
        {goal.health?.letters && goal.health.letters.length > 0 && (
          <View style={styles.healthStrip}>
            <Text style={styles.healthStripLabel}>ALIGN</Text>
            {goal.health.letters.map((l) => (
              <View key={l.letter} style={[styles.healthDot, { backgroundColor: healthLetterBg(l.color) }]}>
                <Text style={styles.healthDotText}>{l.letter}</Text>
              </View>
            ))}
            <Text style={styles.healthScore}>{goal.health.overall_score}%</Text>
          </View>
        )}
